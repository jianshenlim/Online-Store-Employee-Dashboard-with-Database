                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <?php
                                ...
                                if (!empty($_POST['delUserId'])):
                                    $id = $_POST['delUserId'];
                                    $delquery = "delete from user where user_id ='$id'";
                                    $delstmt = $dbh->prepare($delquery);
                                    if (!$delstmt ->execute()){
                                        $err = $delstmt->errorInfo();
                                        echo "<script>alert('Error deleing user record from database - contact System Administrator Error is: $err[2]')</script>";
                                        echo "<script type=\"text/javascript\">location.href = 'viewUser.php';</script>";
                            ...
                            <?php else:
                                ...
                                if(!$stmt ->execute()){
                                    $err = $stmt->errorInfo();
                                    echo "<script>alert('Error displaying user record from database - contact System Administrator Error is: $err[2]')</script>";
                                    echo "<script type=\"text/javascript\">location.href = 'viewUser.php';</script>";
                                };?>

                                <?php if ($stmt -> execute() && $stmt -> rowCount() > 0):
                                    $row = $stmt -> fetchObject()?>
                                    <form method="post" onsubmit="return confirm('Do you want to delete current user?')">
                                        User Id <br>
                                        <input type="hidden" name="delUserId" value="<?= $row -> user_id?>">
                                        <input type="text" value="<?= $row -> user_id?>" disabled><br>
                                        User Name <br>
                                        <input type="text" value="<?= $row -> user_name?>" disabled><br>
                                        <br>
                                        ...
                                        <button type="button" class="btn btn-danger btn-icon-split" onclick="location.href='viewUser.php'">
                                                                    <span class="icon  text-white-50">
                                ...
                                <?php else:?>
                                    User does not exist in database <br>
                                    <br>
                                    <button type="button" onclick="location.href='viewUser.php'">Return</button>
                                <?php endif;?>
                            <?php endif;?>
                        </div>
                    </div>
