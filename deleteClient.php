    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <!-- Page Heading -->
                <h1 class="h3 mb-4 text-gray-800">Delete Client</h1>

                <?php
                if (!empty($_POST['delClientId'])) {
                    $client_id = $_POST['delClientId'];
                    $id_sql = "SELECT * FROM `project` WHERE client_id = '$client_id'";
                    $id_stmt = $dbh-> prepare($id_sql);
                    if (!$id_stmt -> execute()) {
                        $err = $stmt->errorInfo();
                        echo "Error checking from database - contact System Administrator Error is: " . "$err[2] "; ?>
                        <script type="text/javascript">location.href = 'client.php';</script>
                    <?php } else {
                            $clientdata = $id_stmt->fetchAll();
                            $rows = count($clientdata);
                            if ($rows > 0) {
                                echo "Error deleting client as client has existing projects."; ?>
                                <script type="text/javascript">location.href = 'client.php';</script>
                             <?php } else {
                ...
                ?>
                    <form method = "POST" onsubmit="return confirm('Do you really want to delete this client?');">
                        <input type="hidden" name="delClientId" value="<?= $row -> client_id; ?>">
                        Client Id <br/> <input type="text" value="<?= $row -> client_id; ?>" disabled/><br/>
                        First Name <br/> <input type="text" name="client_fname" value="<?= $row -> client_fname; ?>" disabled><br/>
                        Surname <br/> <input  type="text" name="client_sname" value="<?= $row -> client_sname; ?>" disabled><br/>
                        Address <br/> <input type="text" name="client_addr" value="<?= $row -> client_addr; ?>" disabled><br/>
                        Phone <br/> <input  type="text" name="client_phone" value="<?= $row -> client_phone; ?>" disabled><br/>
                        Mobile <br/> <input type="text" name="client_mob" value="<?= $row -> client_mob; ?>" disabled><br/>
                        Email <br/> <input type="email" name="client_email" value="<?= $row -> client_email?>" disabled><br/>
                        Subscribed <input type="checkbox" name="subscribed" value="<?= $row -> subscribed; ?>" <?php if ($row -> subscribed != '0') {
                            echo "checked";
                        }; ?> disabled><br/><br/>
                        ...
                        <button type="button" class="btn btn-danger btn-icon-split" onclick="location.href='client.php'">
                            <span class="icon  text-white-50">
