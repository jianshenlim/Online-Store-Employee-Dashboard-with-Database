    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            if (isset($_SESSION['user_id'])):?>
                ...
                <h1 class="h3 mb-4 text-gray-800">Add New User</h1>
                    <?php
                    $nextId = $dbh->query("SHOW TABLE STATUS LIKE 'user'")->fetch(PDO::FETCH_ASSOC)['Auto_increment'];

                    if (empty($_POST["user_name"])):?>
                        <form method = "post" onsubmit="return checkPass(this);">
                            User Id <br>
                            <input type="text" name = "user_id" value = <?php echo $nextId ?> disabled/><br/>
                            User Name <br>
                            <input type="text" name ="user_name" maxlength="64" required><br/>
                            User Password <br>
                            <input type="text" name ="user_pass" maxlength="64" required><br/>
                            Re-Enter Password <br>
                            <input type="text" name ="user_pass_check" maxlength="64" required><br/>
                            <br>
                            ...
                            <button type="button"  class="btn btn-danger btn-sm btn-icon-split" onclick="location.href='viewUser.php'">
                                <span class="icon  text-white-50">
                    ...
                    <?php else:
                        ...
                        $new_user_pass = hash('sha256',$_POST["user_pass"]);

                        $sql = "INSERT INTO user (user_id,user_name,user_pass) VALUES ('$nextId','$new_user_name','$new_user_pass')";
                        $stmt = $dbh -> prepare($sql);
                        ?>
                        <?php if (!$stmt -> execute()):
                            $err = $stmt->errorInfo();
                            echo "<script>alert('Error adding user record to database - contact System Administrator Error is: $err[2]')</script>";
                            echo "<script type=\"text/javascript\">location.href = 'addUser.php';</script>"; ?>

                        <?php else:?>
                            <a> New User has been added</a>
                            <form>
                                User Id <br>
                                <input type="text" name = "user_id" value = <?= $nextId ?> disabled/><br/>
                                User Name <br>
                                <input type="text" name ="user_name" value = <?= $new_user_name ?> disabled/><br/>
                                <br>

                                <button type="submit" style=" margin-right: 1em;" class="btn btn-sm btn-primary btn-icon-split " onclick="location.href='addUser.php'">
                                <span class="icon  text-white-50">
                                        <i class="fas fa-plus"></i>
                                    </span>
                                    <span class="text">Add Another</span>
                                </button>
                                <button type="button"  class="btn btn-sm btn-primary btn-icon-split" onclick="location.href='viewUser.php'">
                                    <span class="icon  text-white-50">
        ...
        <footer class="sticky-footer bg-white">
            <div class="container text-center my-auto">
                <button type="button" style="text-align: center" class="btn btn-lg btn-info " onclick="window.open('displayCode.php?section=user')">
