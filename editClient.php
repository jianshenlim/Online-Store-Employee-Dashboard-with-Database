            ...
            <?php
                ...
                <h1 class="h3 mb-4 text-gray-800">Edit Client</h1>
                    ...
                    if (empty($_POST["client_fname"])) { ?>
                        <form method = "post">
                            Client Id <br/> <input type="text" name="client_id" value="<?= $row -> client_id; ?>" disabled/><br/>
                            First Name <br/> <input type="text" name="client_fname" value="<?= $row -> client_fname; ?>" required maxlength="64"><br/>
                            Surname <br/> <input type="text" name="client_sname" value="<?= $row -> client_sname; ?>" required maxlength="64"><br/>
                            Address <br/> <input type="text" name="client_addr" value="<?= $row -> client_addr; ?>" required><br/>
                            Phone <br/> <input type="text" name="client_phone" value="<?= $row -> client_phone; ?>" required maxlength="10"><br/>
                            Mobile <br/> <input type="text" name="client_mob" value="<?= $row -> client_mob; ?>" required maxlength="10"><br/>
                            Email <br/> <input type="email" name="client_email" value="<?= $row -> client_email; ?>" required maxlength="256"><br/>
                            Subscribed <input type="checkbox" name="subscribed" value="1" <?php if ($row -> subscribed != '0') {
                                echo "checked";
                            }; ?> ><br/><br/>
                            ...
                            <button type="button"class="btn btn-sm btn-danger btn-icon-split" onclick="location.href ='client.php'">
                                <span class="icon  text-white-50">
                    ...
                    <?php }
                        ...
                        $sql = "UPDATE `client` SET `client_fname`=:new_client_fname, `client_sname`=:new_client_sname, `client_addr`=:new_client_addr, `client_phone`=:new_client_phone, `client_mob`=:new_client_mob, `client_email`=:new_client_email, `subscribed`=:new_client_sub WHERE `client_id`=:client_id";
                        $pdoResult = $dbh-> prepare($sql);
                        if (!$pdoResult-> execute(array(":new_client_fname"=>$new_client_fname, ":new_client_sname"=>$new_client_sname, ":new_client_addr"=>$new_client_addr, ":new_client_phone"=>$new_client_phone, ":new_client_mob"=>$new_client_mob, ":new_client_email"=>$new_client_email, ":new_client_sub"=>$new_client_sub,":client_id"=>$client_id))) {
                            $err = $pdoResult->errorInfo();
                            echo "Error editing client record in database - contact System Administrator Error is: " . "$err[2] ";
                        } ?>

                        <script type="text/javascript">location.href ='client.php';</script>
                    <?php } ?>
                    <?php else:?>
