    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <?php
                ...
                if (!$stmt ->execute()){
                    $err = $stmt->errorInfo();
                    echo "<script>alert('Error retrieving category record from database - contact System Administrator Error is: $err[2]')</script>";
                ...
                if (empty($_POST["category_name"])):?>
                    <form method = "post" onsubmit="return confirm('Do you really want to edit this category?')">
                        Category Id <br>
                        <input type="text" name = "category_id" value = "<?= $row -> category_id; ?>" disabled/><br/>
                        Category Name <br>
                        <input type="text" name = "category_name" value = "<?= $row -> category_name; ?>" maxlength="64" required><br/>
                        <br>
                        ...
                        <button type="button"class="btn btn-sm btn-danger btn-icon-split" onclick="location.href ='category.php'">
                                                                    <span class="icon  text-white-50">
                ...
                <?php else:
                    ...
                    if (!$pdoResult-> execute(array(":new_category_name"=>$new_category_name,":category_id"=>$category_id))):
                        $err = $pdoResult->errorInfo();
                        echo "<script>alert('Error editing category record in database - contact System Administrator Error is: $err[2]')</script>";
                        echo "<script type=\"text/javascript\">location.href = 'category.php';</script>";
