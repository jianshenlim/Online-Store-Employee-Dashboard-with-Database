...
<head>
    ...
    <title>MOW | Email Client(s)</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <!-- Page Heading -->
                <h1 class="h3 mb-4 text-gray-800">Email Selected Client(s)</h1>
                ...
                if (!empty($_POST["clientIds"]) ) {
                    ...
                    foreach ($_POST['clientIds'] as $client_id) {
                        $query = "SELECT client_fname, client_sname, client_email FROM client WHERE client_id = '$client_id'";
                        $stmt = $dbh->prepare($query);
                        $stmt ->execute();
                        $row = $stmt -> fetchObject();

                        $concat = $row->client_fname . " " . $row->client_sname . " " ."<" . $row->client_email . ">";
                        $clients[] = $concat;
                    }

                    $to_clients = implode(", ", $clients); ?>

                    <form method = "POST">
                        <input type="hidden" name="client_email" value="<?= $to_clients; ?>">
                        To: <br/> <input type="text" value="<?php echo $to_clients ?>" disabled><br/>
                        Subject: <br/><input type="text" name="email_subject" required></br>
                        Text: <br/><textarea name="email_text" required></textarea>
                        ...
                        <button type="button" class="btn btn-danger btn-icon-split" onclick="location.href='subscribedClients.php'">
                            <span class="icon  text-white-50">
                    ...
                    if (isset($_POST['email_subject']) && isset($_POST['email_text'])) {
                        ...
                        $text = addslashes($_POST["email_text"]);
                        $header = "From: Jack Trader <jackt@masterofwedding.com>";

                            $sent = mail($to, $subject, $text, $header);
                            if ($sent == true) {
                                echo "Email was successfully sent"; ?>
                                <script type="text/javascript">location.href ='subscribedClients.php';</script>
                                <?php
                            } else {
                    ...
                    <?php
                                ...
                                echo "No emails selected, subject or text is set."; ?>
                                <script type="text/javascript">location.href ='subscribedClients.php';</script>
                <?php
                        }
                    }
