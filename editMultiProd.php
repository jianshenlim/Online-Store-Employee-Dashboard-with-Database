...
<head>
    ...
    <title>MOW | Edit Multiple Products</title>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            if (isset($_SESSION['user_id'])):?>
                <!-- Page Heading -->
                <h1 class="h3 mb-2 text-gray-800">Edit Multiple Products</h1>
                <!-- DataTales Example -->
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <form method = "post" onsubmit="return confirm('Do you really want to edit selected product(s)?')">
                            <?php
                                if ($_SERVER['REQUEST_METHOD']==='POST' && empty($_POST['prodId'])){
                                    echo "<script>alert('No Products Selected')</script>";
                                    echo "<script type=\"text/javascript\">location.href = 'editMultiProd.php';</script>";

                                }

                                if ($_SERVER['REQUEST_METHOD']==='POST' && !empty($_POST['prodId'])){
                                    foreach ($_POST['prodId'] as $prodId){
                                        if (isset($_POST['purchase_price'][$prodId])){
                                            $updateQuery1 = "update `product` set `purchase_price`=:purchase_price where `product_id`=:prodId";
                                            $uStmt1 = $dbh -> prepare($updateQuery1);
                                            if(!$uStmt1 -> execute([
                                            ":purchase_price" => $_POST['purchase_price'][$prodId],
                                            ":prodId" => $prodId
                                            ])){
                                                echo "<script>alert('Error occurred while updating purchase price of product ID: $prodId')</script>";
                                                break;
                                            }
                                        }else{
                                            echo "<script>alert('Product ID: $prodId is missing a purchase price value')</script>";
                                            break;
                                        }

                                if (isset($_POST['sale_price'][$prodId])){
                                    $updateQuery2 = "update `product` set `sale_price`=:sale_price where `product_id`=:prodId";
                                    $uStmt2 = $dbh -> prepare($updateQuery2);
                                        if(!$uStmt2 -> execute([
                                        ":sale_price" => $_POST['sale_price'][$prodId],
                                        ":prodId" => $prodId
                                        ])){
                                            echo "<script>alert('Error occurred while updating sale price of product ID: $prodId')</script>";
                                            break;
                                    }
                                }else{
                                    echo "<script>alert('Product ID: $prodId is missing a sale price value')</script>";
                                    break;
                                }
                            }
                    }
                    ...
                    if (!$stmt -> execute()){
                        $err = $stmt->errorInfo();
                        echo "<script>alert('Error retrieving product records in database - contact System Administrator Error is: $err[2]')</script>";
                                    ...
                                    <tbody>
                                        <?php  while($row = $stmt -> fetchObject()): ?>
                                            <tr>
                                                ...
                                                <td><input type = "number" name = "purchase_price[<?= $row->product_id; ?>]" value="<?php echo $row -> purchase_price; ?>" required></td>
                                                <td><input type = "number" name = "sale_price[<?= $row->product_id; ?>]" value = "<?php echo $row -> sale_price; ?>" required</td>
                                                <td style="text-align: center"><input type="checkbox" name="prodId[]" value="<?php echo $row->product_id; ?>"/></td>
        ...
        <footer class="sticky-footer bg-white">
            <div class="container text-center my-auto">
                <button type="button" style="text-align: center" class="btn btn-lg btn-info " onclick="window.open('displayCode.php?section=multiprod')">
                    <span class="text">Multiple Products</span>
                </button>
            </div>
        </footer>
        <!-- End of Footer -->
